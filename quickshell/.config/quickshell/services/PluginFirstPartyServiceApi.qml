...
QtObject {
  ...
  function setIdleEnabled(value) {
    if (serviceId === "swayp.idle" && _setIdleEnabled) _setIdleEnabled(!!value)
  }

  function setNightlight(value) {
    if (serviceId === "swayp.nightlight" && _setNightlight) _setNightlight(!!value)
  }

  function setDoNotDisturb(value) {
    if (serviceId === "swayp.notifications" && _setDoNotDisturb) _setDoNotDisturb(!!value)
  }

  function runAction(action, showFeedback, playerId) {
    if (serviceId === "swayp.media" && _runAction)
  ...
  function selectPlayer(playerId) {
    if (serviceId === "swayp.media" && _selectPlayer) _selectPlayer(String(playerId || ""))
  }
}
